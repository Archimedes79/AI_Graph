...
import { InputPickerWidget, WIDGETS } from '../../widgets/roster.ts';
 ...
 */
export class GuiNode extends GraphNodeElement<GuiConfig> {
  ...
  override referencedPaths(node: GraphNode): string[] {
    ...
    for (const widget of this.config(node).widgets) {
      const element = BY_KIND.get(widget.kind);
      if (!(element instanceof InputPickerWidget)) continue;
