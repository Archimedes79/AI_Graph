...
const GRAPH_SYSTEM =
  'You are an expert at authoring Graph DSL documents for a visual node-based AI workflow tool. When asked to '
  + 'design a graph, output ONLY a fenced ```json code block containing a complete Graph DSL document, followed '
  + 'by a brief explanation outside the block. Do not add extra prose before the code block.\n\n'
  ...
  + '}\n\n'
  + 'Valid node_type values: input, data, ai, code, output, gui. An "input" node\'s config.input_mode selects text, '
  + 'file, or directory input. A "data" node is persisted graph memory with one optional input port named "input" '
  + 'and one output port named "output". Define data nodes before code or ai nodes when a workflow has known '
  + 'intermediate contracts. Set config.data_format to text or structure; put the precise schema, field names, '
  + 'types, nesting, and constraints in config.data_format_prompt; and initialize config.data_value when useful. '
  + 'Connected code and ai nodes must honor those source and target contracts. There is no dedicated merge/split '
  + 'node type: fan-in (multiple edges into one multi input port) and fan-out (one output wired to many inputs) are '
  + 'pure edge wiring, and any merge/split-style aggregation (concat/sum/count/json_list a set of inputs, or '
  + 'splitting text into a list) should be written as a "code" node. For a display-only output, use node_type '
  + '"output" with config.write_mode = "window" (shows the result in a text window). Every node must declare its '
  + 'own inputs and outputs port arrays, even if empty, and every port id must be unique within its node. Edges '
  + 'must reference existing node ids and port ids declared on those nodes.';

/** Ask for a whole Graph DSL document from a description. The caller parses it. */
export async function generateGraph(
  description: string, context: string, deps: Pick<GenerateDeps, 'ai' | 'target'>,
