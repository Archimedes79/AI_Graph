...
describe('the two halves of every element', () => {
  it('are registered for the same node types and widget kinds', () => {
    ...
    expect(Object.keys(WIDGET_BUILDERS).sort()).toEqual([...registry.widgetKinds()].sort());
  });
  ...
  it.each([...registry.widgetKinds()])('widget %s: <Kind>WidgetRunner, <Kind>WidgetGuiBuilder and <Kind>WidgetView', (kind) => {
